...
Ce graphique a pour but de déterminer les occurrences des différents types de proies consommées dans chaque zone étudiée.

Nous voyons ici clairement que dans certaines zones (comme ElkhornSlough) la diversité des types de proies consommées est faible tandis que dans d'autres zones (comme Monterey et BigSur) le type de proie consommée est beaucoup plus diversifié et homogène.
...
Ce graphique illustre l'évolution de la masse du corps des loutres en fonction de la variation de longueur de leur corps pour les différents types de régimes observés.

On observe différentes pentes pour les différents régimes alimentaires, le régime alimentaire associé à la pente la plus forte est celui à base de moule ce qui pourrait indiquer que ce régime alimentaire est associé à la plus grande prise de masse corporelle chez l'espèce étudiée.

Tandis que le régime alimentaire à base d'oursin semble être associé à la plus petite prise de masse corporelle chez cette espèce.
...
On observe une utilisation marquée des outils lorsque la proie principale est l’escargot, qui se distingue nettement des autres catégories. En revanche, les loutres recourent rarement aux outils pour accéder aux ormeaux, palourdes, crabes, moules, et encore moins aux oursins.
...
Ce graphique illustre le logarithme de la fréquence d’utilisation d’outils par les loutres en fonction de leur classe d'age.

Nous constatons que l'utilisation d'outils est plus élevée chez les adultes âgés, ceci pourrait démontrer que c'est une capacité acquise et non innée.
...
Nous constatons une valeur extrême pour la classe adulte âgée (AA).
...
Nous voyons ici que notre modèle linéaire est globalement significatif au seuil alpha de 5%. De plus, l'effet du pourcentage de moules consommées ainsi que celui des escargots consommés sont significativement différents de 0 au seuil alpha de 5%. Le R^2^ ajusté est plutôt correct avec une valeur de 0.8551.

Concernant l'analyse des résidus :

-   L'étendue des résidus est égale à celle des valeurs prédites (A).

-   La linéarité est respectée avec un léger décalage sur le centre des valeurs prédites peut-être dû à un manque d'observations sur cette partie du graphique par rapport aux extrémités (A).

-   Les résidus ont une distribution qui ne diffère pas trop de la normale avec malgré tout une asymétrie sur les valeurs faibles et fortes (B).
...
-   Nous observons plusieurs valeurs possédant une distance Cook importante ainsi qu'un effet de levier important. Certaines valeurs possèdent juste une distance de Cook importante et d'autres ne possèdent qu'un effet de levier important (D).

Sur le graphique, nous pouvons observer que la distribution des observations n'est pas homogène. Beaucoup sont situées sur la gauche du graphique et sur la droite mais le centre de celui-ci est moins fourni en observations.

De plus, nous observons que lorsque la fréquence d'utilisation d'outils augmente, le pourcentage de moules consommées est assez faible comparé au pourcentage d'escargots consommés qui lui, est clairement élevé pour une utilisation d'outils fréquente.
...
Le R² ajusté est moyen et explique 43 % de la variabilité de l'apport calorifique.
...
-   Nous observons quelques valeurs possédant un effet de levier important. Cependant, ces points n'ont pas une distance de Cook importante (D).
...
Dans un premier temps, il est vérifié que la variable réponse soit bien numérique et que la distribution soit bien (globalement) normale.

Ensuite, le test du modèle avec interactions révèle que de multiples variables apparaissent significativement différentes de 0 au seuil alpha de 5% via les tests t de Student.

L'examen du modèle sans interactions montre toutefois une valeur de R carré très proche.

Nous réalisons une ANOVA de comparaison des deux modèles (celui avec et sans interaction) pour vérifier le choix de modèle. On observe que les deux modèles sont significativement différents l'un de l'autre au seuil alpha de 5% ce qui nous mène à choisir le modèle le plus complexe soit celui avec interactions (otters_lm3).
...
Le R² ajusté est moyen et explique 52 % de la variabilité du nombre de proies consommées.

Il n'est pas constaté de différences significatives du nombre de proies consommées entre mâles et femelles.

Les classes d'âge adultes et adultes âgés sont significatives ou presque au seuil alpha de 5%, ce qui suggère donc une différence du nombre de proies consommées par classe d'âge.

L'utilisation d'outils augmente le nombre de proies consommées pour les loutres qui utilisent des outils pour au moins 3% de leur consommation de proies.

Les résidus ne varient pas trop des valeurs prédites.

Une non-normalité des résidus assez présente est observée.

Nous considérons que ce modèle valide le critère d'homoscédasticité.

Plusieurs valeurs extrêmes avec des effets de leviers élevés et des distances de Cook importantes sont constatées.
...
Nous avons testé différentes formes de GLM, nous nous assurons ici de la non-normalité de la distribution dans le cas de la variable réponse "urchin_num" (quantité d'oursins consommés).

Nous choisissons initialement une famille de type "poisson" car la situation correspond à un dénombrement, une abondance. En regardant la dispersion observée (déviance résiduelle/degrés de libertés des résidus) nous nous rendons compte que nous sommes dans une situation de surdispersion, nous optons alors pour un quasipoisson : nous observe que le paramètre de dispersion ne change pas même si la surdispersion est moins marquée mais que les valeurs p des test Z ont diminué indiquant que les variables ont des valeurs qui sont d'avantage significativement différente de 0 au seuil alpha de 5%.

Les modèles GLM de famille quasi étant moins robustes, cette augmentation de la significativité peut n'être que due à cette diminution de robustesse donc le modèle ne semble pas plus pertinent.
...
Nous choisissons une distribution gaussienne car notre variable est strictement positive, continue et positive.

Nous avons testé différentes formes de GLM, nous nous assurons ici de la légère non-normalité de la distribution dans le cas de la variable réponse "Epreycalc" (nombre total de calories consommées).

La déviance des résidus est très élevée ce qui remet en cause notre modèle.

Toutes les variables sont significatives au seuil alpha 5%.

Nous constatons une valeur extrême avec un effet de levier important ainsi que d'autres valeurs avec des distances de Cook importantes.
...
-   Le modèle linéaire 1 possède un R² ajusté de 0.8, il explique donc 80 % de la variation de la masse. Les résidus de ce modèle varient fort par rapport aux valeurs prédites, l'homoscédasticité n'est pas valide, il y a une non-normalité et des valeurs extrêmes. Si nous voulons utiliser ce modèle, il faudrait envisager une GLM ou une modification des variables avec des fonctions telles que le logarithme et faire attention aux valeurs extrêmes.
-   Le modèle linéaire 2 possède un R² ajusté moyen, qui explique 52 % de la variabilité du nombre de proies consommées. Les résidus de ce modèle ne varient pas trop des valeurs prédites, l'homoscédasticité est raisonnablement valide, mais il y a une non-normalité et des valeurs extrêmes. Si nous voulons utiliser ce modèle, il faudrait envisager une GLM et faire attention aux valeurs extrêmes.
-   Le modèle otters_glm1 explique la variation du nombre d'oursins consommés par les variables tool_use (fréquence d'utilisation d'outils), length (longueur de la loutre) et ageclass (classe d'âge). Il y a surdispersion des résidus, donc nous faisons un modèle quasi-poisson, mais la surdispersion reste importante. Ce modèle ne peut donc pas être utilisé ainsi et étoffer le jeu de données pourrait être une piste d'avancement.
-   Le modèle otters_glm2 explique que les variables hardness (dureté de la proie), tool_use (fréquence d'utilisation d'outils) et l'âge expliquent la variation de l'apport calorifique total. Comme nous utilisons une GLM la non-normalité des résidus n'affecte pas le modèle. Il faut faire attention aux valeurs extrêmes présentes.
