...
-   Le modèle otters_glm1 explique la variation du nombre d'oursins consommés par les variables tool_use (fréquence d'utilisation d'outils), length (longueur de la loutre) et ageclass (classe d'âge). Il y a surdispersion des résidus et même en faisant la version quasi du modèle ceci ne change pas. Les Ce modèle ne peut donc pas être utilisé ainsi et etoffer le jeu de données pourrait être une piste d'avancement.
