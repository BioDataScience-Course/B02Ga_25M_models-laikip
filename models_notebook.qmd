...
Dans un premier temps, il est vérifié que la variable réponse soit bien une numérique et que la distribution soit bien (globalement) normale.
...
Les classes d'âge adultes et adultes âgés sont significatives ou presque au seuil alpha 5%, ce qui suggère donc une différence du nombre de proies consommées par classe d'âge.
...
-   Le modèle otters_glm2 explique que les variables hardness (dureté de la proie), tool_use (fréquence d'utilisation d'outils) et l'âge explique la variation de l'apport calorifique total. Comme nous utilisons une GLM la non-normalité des résidus n'affecte pas le modèle. Il faut faire attention aux valeurs extrêmes présentes.
