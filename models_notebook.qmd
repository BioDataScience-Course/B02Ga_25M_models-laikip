...
Notre R² ajusté est moyen et explique 52 % de la variabilité du nombre de proies consommées.
...
Nous choisissons une distribution gaussienne car notre variable est strictement positive, continu et positif.
...
La déviance des résidus est très élevé ce qui remet en cause notre modèle.

Toutes les variables sont significatives au seuil alpha 5%.

Nous constatons une valeur extrêmes avec un effet levier important ainsi que d'autres valeurs avec des distances de Cook importante.
...
-   Le modèle linéaire 1 possède un R² ajusté de 0.8 donc il explique 80% de la variation de la masse. Les résidus de ce modèle varient fort par rapport valeurs prédites, l'homoscédasticité n'est pas valide, il y a une non-normalité et des valeurs extrêmes. Si nous voulons utiliser ce modèle il faudrait envisager une glm ou une modification des variables avec des fonctions tel que le logarithme et faire attention aux valeurs extrêmes.
...
-   Le modèle otters_glm2 explique que les variables hardness(dureté de la proie), tool_use(fréquence d'utilisation d'outils) et l'age explique la variation de l'apport calorifique totale. Comme nous utilisons une glm la non-normalité des résidus n'affecte pas le modèle. Il faut faire attention aux valeurs extrêmes présentes.
